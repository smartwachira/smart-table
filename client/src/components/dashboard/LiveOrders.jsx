...
import { 
    ...
    User, BellRing, Flame, ArrowRight, UtensilsCrossed, Banknote
} from 'lucide-react';
...
export default function LiveOrders() {
    ...
    // ⚡ SECURE CASH COLLECTION HANDLER
        ...
        return (
            <div ref={isFirst ? newOrderScrollRef : null} className={`flex flex-col rounded-2xl shadow-sm border-2 overflow-hidden transition-all hover:shadow-md ${slaColor}`}>
                ...
                <div className="px-3 md:px-4 py-2 border-b border-slate-100 bg-white/50 flex justify-between items-center">
                    ...
                    {isCashPending ? (
                        ...
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-emerald-100 text-emerald-800 border border-emerald-200 rounded-lg text-[10px] md:text-xs font-black uppercase tracking-wider">
                            <CheckCircle2 size={12} /> Paid ({order.payment_method})
                        </span>
                    )}
                </div>
                ...
                <div className="p-3 md:p-4 flex-1 space-y-3 bg-white/40">
                    {order.OrderItems?.map((item, idx) => (
                        <div key={idx} className="flex gap-2 md:gap-3 text-xs md:text-sm">
                            ...
                            <div>
                                <span className="font-bold text-slate-800 text-sm md:text-base">{item.MenuItem?.name || item.name}</span>
                ...
                <div className="p-2 md:p-3 bg-white/60 border-t border-slate-100 flex flex-col gap-2">
                    {/* ⚡ Conditionally render the Cash Collection button for waiters/managers */}
                    {isCashPending && ['MANAGER', 'OWNER', 'STAFF'].includes(userRole) && (
                        ...
                        >
                            <Banknote size={16} /> Mark Cash Collected
                        </button>
                    )}
                    ...
                    <div className="flex gap-2">
                        ...
                        {order.status === "READY" && (
                            <button 
                                className="flex-1 flex items-center justify-center gap-1.5 md:gap-2 bg-emerald-500 hover:bg-emerald-600 text-white py-2.5 md:py-3 px-2 rounded-xl text-xs md:text-sm font-bold transition-all active:scale-95 shadow-sm"
                                onClick={() => updateStatus(order.order_id, 'COMPLETED')}
                            >
                                <UtensilsCrossed size={16}/> Complete / Served
                            </button>
                        )}
    ...
    return (
        <div className="max-w-[1600px] mx-auto p-3 md:p-6 lg:h-screen flex flex-col bg-slate-100 overflow-hidden">
            
            {/* Header */}
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200 mb-4 md:mb-6 shrink-0 gap-3">
                <div>
            ...
            {orders.length === 0 ? (
                ...
                /* ⚡ Snap-Scrolling Mobile Layout -> Expanding to 3-Column Grid on Desktop */
                <div className="flex-1 flex flex-row lg:grid lg:grid-cols-3 gap-4 md:gap-6 overflow-x-auto lg:overflow-hidden snap-x snap-mandatory pb-4 lg:pb-0 custom-scrollbar">
                    
                    {/* Column 1: PENDING */}
                    <div className="flex flex-col bg-slate-200/50 rounded-[2rem] lg:rounded-3xl border border-slate-200 overflow-hidden min-w-[85vw] sm:min-w-[400px] lg:min-w-0 snap-center shrink-0 lg:shrink h-full max-h-[75vh] lg:max-h-full">
                    ...
                    {/* Column 2: PREPARING */}
                    <div className="flex flex-col bg-indigo-50/50 rounded-[2rem] lg:rounded-3xl border border-indigo-100 overflow-hidden min-w-[85vw] sm:min-w-[400px] lg:min-w-0 snap-center shrink-0 lg:shrink h-full max-h-[75vh] lg:max-h-full">
                    ...
                    {/* Column 3: READY */}
                    <div className="flex flex-col bg-emerald-50/50 rounded-[2rem] lg:rounded-3xl border border-emerald-100 overflow-hidden min-w-[85vw] sm:min-w-[400px] lg:min-w-0 snap-center shrink-0 lg:shrink h-full max-h-[75vh] lg:max-h-full">
